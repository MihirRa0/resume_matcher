...
def create_gemini_prompt(resume_text: str, jd_text: str) -> str:
    ...
    **Output Format:**
    ...
    - `recommended_courses`: A JSON array of simple strings. For example: ["Data Science with Python", "Advanced SQL for Data Analysts"].
    - `project_suggestions`: A JSON array of simple strings. For example: ["Build a sales prediction dashboard", "Analyze customer churn for a subscription service"].
